<script>
	...
	// Cache management
	const CACHE_KEY = 'screens_leaderboard_cache';
	...
	const ZOOM_STATE_KEY = 'screens_zoom_state';
	...
	onMount(() => {
		...
		if (typeof window !== 'undefined') {
			window.addEventListener('screens-leaderboard-invalidate', handleCacheInvalidation);

			// Cleanup listener on destroy
			return () => {
				window.removeEventListener('screens-leaderboard-invalidate', handleCacheInvalidation);
			};
		}
	});
	...
	async function loadLeaderboard() {
		...
		try {
			...
			const response = await fetch(`/api/screens/leaderboard?${params}`);
			if (response.ok) {
...
<div class="{fullscreen ? 'w-full h-full bg-black text-white flex flex-col' : 'min-h-screen bg-black text-white flex flex-col items-center justify-center p-4'}">
	<!-- Leaderboard -->
	<div class="{fullscreen ? 'w-full h-full' : 'w-full max-w-7xl h-full relative'}">
		...
		<div class="grid grid-cols-2 gap-6 h-full {currentZoomState === 'zoomed' ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}">
				{#each ['easy', 'medium', 'hard', 'very hard'] as difficultyKey}
					{@const players = Array.isArray(leaderboardsByDifficulty[difficultyKey]) ? leaderboardsByDifficulty[difficultyKey] : []}
						...
						<div class="p-2 flex-1 overflow-hidden flex flex-col">
							{#if !players || players.length === 0}
								<div class="text-center py-8 text-gray-400 flex-1 flex flex-col justify-center">
									...
									<p class="text-sm mt-2">Zagraj kilka screenów!</p>
								</div>
							{:else}
		...
		{#if currentZoomState === 'zoomed' && currentZoomedDifficulty}
			{@const players = Array.isArray(leaderboardsByDifficulty[currentZoomedDifficulty]) ? leaderboardsByDifficulty[currentZoomedDifficulty] : []}
					...
					<div class="p-2 flex-1 overflow-hidden flex flex-col">
						{#if !players || players.length === 0}
							<div class="text-center py-8 text-gray-400 flex-1 flex flex-col justify-center">
								...
								<p class="text-xl mt-2">Zagraj kilka screenów!</p>
							</div>
						{:else}
